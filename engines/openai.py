...
class ChatgptTranslate(GenAI):
    ...
    def __init__(self):
        ...
        self.context_rounds = self.config.get('context_rounds', 0)
    ...
    def get_body(self, text):
        ...
        if self.context_rounds > 0:
            history = self.conversation_history[-(self.context_rounds * 2):]
            body['messages'].extend(history)
    ...
    def add_to_history(self, user_content, assistant_content):
        if self.context_rounds > 0:
            self.conversation_history.append(
                {'role': 'user', 'content': user_content})
            self.conversation_history.append(
                {'role': 'assistant', 'content': assistant_content})
            # Keep only the required history rounds
            max_history = self.context_rounds * 2
