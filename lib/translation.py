...
class Translation:
    ...
    def translate_paragraph(self, paragraph):
        ...
        if isinstance(translation, GeneratorType):
            # Show streaming output if it's a single translation or running sequentially.
            if self.total == 1 or self.translator.concurrency_limit == 1 \
                    or getattr(self.translator, 'context_rounds', 0) > 0:
                temp = ''
                clear = True
        ...
        self.translator.add_to_history(text, paragraph.translation)
    ...
    def handle(self, paragraphs=[]):
        ...
        # Force sequential processing if context_rounds is enabled.
        concurrency_limit = self.translator.concurrency_limit
        if getattr(self.translator, 'context_rounds', 0) > 0:
            concurrency_limit = 1
