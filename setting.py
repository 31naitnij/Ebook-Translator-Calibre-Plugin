...
class TranslationSetting(QDialog):
    ...
    def layout_engine(self):
        ...
        context_rounds = QSpinBox()
        context_rounds.setRange(0, 5)
        genai_layout.addRow(_('Context Rounds'), context_rounds)
        ...
        context_rounds.valueChanged.connect(
            lambda value: self.current_engine.config.update(
                context_rounds=value))
        ...
        def show_genai_preferences(config):
            ...
            # Context Rounds
            context_rounds.setValue(
                config.get('context_rounds', self.current_engine.context_rounds))
            genai_group.setVisible(True)
